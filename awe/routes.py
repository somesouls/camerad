...
#   - Logika inti (nik_stats & nik_backfill) ADA DI avaya/db.py demi kerapian
#     lapisan; handler di bawah hanya pembungkus HTTP.
#   - Selalu mengembalikan JSON (dibungkus try/except) supaya UI tidak kena
#     'Unexpected token S' saat server error.
# =============================================================
async def awe_nik_stats():
    def _do():
        conn = avdb.init_db(avdb.connect())
        try:
            return avdb.nik_stats(conn)
        finally:
            conn.close()
    try:
        d = await run_in_threadpool(_do)
...
async def awe_nik_backfill(request: Request):
    def _do():
        conn = avdb.init_db(avdb.connect())
        try:
            res = avdb.nik_backfill(conn)
            res["stats"] = avdb.nik_stats(conn)
            return res
        finally:
            conn.close()
    try:
