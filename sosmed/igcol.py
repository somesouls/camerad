...
def extract_ig_comments(obj, media_id=None, code=None, official=None,
    ...
    """Parser Universal: Telusuri rekursif payload JSON Instagram secara universal."""
    ...
    # Batasi kedalaman agar tidak infinity loop pada JSON raksasa
    if depth > 50 or not isinstance(obj, (dict, list)):
        return results

    if isinstance(obj, dict):
        # Abaikan node media/postingan (caption) agar tidak disangka komentar
        if any(k in obj for k in ("media_type", "product_type", "carousel_media")):
            pass
        else:
            # DETEKSI KOMENTAR: Apakah objek ini terlihat seperti komentar IG?
            is_comment = (
                "text" in obj and 
                "user" in obj and isinstance(obj["user"], dict) and "username" in obj["user"] and
                ("created_at" in obj or "created_at_utc" in obj) and
                ("pk" in obj or "id" in obj or "comment_id" in obj)
            )
        ...
        # Telusuri ke dalam dictionary
        for v in obj.values():
            extract_ig_comments(v, media_id, code, official, results, seen, depth + 1)
            
    elif isinstance(obj, list):
        # Telusuri ke dalam array/list
        for item in obj:
            extract_ig_comments(item, media_id, code, official, results, seen, depth + 1)
            
    return results

def _scroll_ig_comments(page):
    """Scroll bertahap ke bawah pada panel komentar agar Lazy-Load API terpicu."""
    try:
        page.evaluate("""() => {
            const scrollers = Array.from(document.querySelectorAll('div, ul, section, main, article'))
                .filter(el => el.scrollHeight > el.clientHeight + 100);
            for (let s of scrollers) {
                s.scrollBy({top: 800, left: 0, behavior: 'smooth'});
            }
...
def _click_ig_replies(page):
    """Mengeklik tombol balasan dengan penanda khusus agar tidak dobel klik."""
    try:
        clicked = page.evaluate("""() => {
            let count = 0;
            const btns = Array.from(document.querySelectorAll('[role="button"], span, div, a')).filter(el => {
                const text = (el.textContent || '').toLowerCase().trim();
                // Lewati tombol sembunyikan balasan
                if (text.includes('hide') || text.includes('sembunyi') || text.includes('tutup')) return false;
                
                return (text.includes('view') && text.includes('repl')) ||
                       (text.includes('lihat') && text.includes('balasan')) ||
                       (text.includes('view') && text.includes('more')) ||
                       text.includes('lihat lainnya') || text.includes('selengkapnya');
            });
            
            for (let b of btns) {
                // KUNCI: Jangan klik tombol yang sudah diklik sebelumnya
                if (b.dataset.sosmedClicked) continue;
                ...
                if (style.display !== 'none' && style.visibility !== 'hidden' && b.offsetHeight > 0) {
                    try { 
                        ...
                        b.click(); 
                        b.dataset.sosmedClicked = "true"; // Beri tanda sudah diklik
                        count++; 
                    } catch(e) {}
                    if(count >= 10) break;
                }
...
() => {
  const out = [];
  const seen = new Set();
  const times = document.querySelectorAll('a[href*="/c/"] time[datetime]');
  
  for (const te of times) {
    const a = te.closest('a');
    if (!a) continue;
    ...
    // Cari Username dan Teks dengan lebih akurat
    for(let i=0; i<6; i++) {
        if(!container) break;
        
        // 1. Ambil Username dari link profil (paling akurat)
        const userLink = container.querySelector('h3 a[href^="/"], a[role="link"][href^="/"]');
        if (userLink) {
             const hm = (userLink.getAttribute('href') || '').match(/^\/([^\/?#]+)/);
             handle = hm ? hm[1] : (userLink.textContent || '').trim();
             
             // Bersihkan dari label tambahan IG seperti "Sudah Diverifikasi" atau "Verified"
             handle = handle.replace(/(Sudah Diverifikasi|Verified)/gi, '').trim();
        }
        
        // 2. Ambil Teks dari container (Buang username dan metadata)
        const allSpans = Array.from(container.querySelectorAll('span[dir="auto"], div[dir="auto"]'));
        
        for (const span of allSpans) {
             let txt = (span.textContent || span.innerText || '').trim();
             
             // Abaikan jika teksnya adalah username, waktu (1 hari, 2 jam), tombol Reply/Like, dll
             if (txt && 
                 txt !== handle && 
                 txt.length > 3 && 
                 !txt.match(/^(Reply|View replies|Like|Suka|Balas|Lihat|Translate|Terjemahkan)$/i) &&
                 !txt.match(/^(\d+\s*(hari|jam|menit|detik|minggu|d|h|m|s|w)\s*(lalu|yang lalu)?)$/i) &&
                 !txt.toLowerCase().includes('sudah diverifikasi') &&
                 !txt.toLowerCase().includes('likes')) {
                 
                 // Ini adalah teks komentarnya!
                 text = txt;
                 break;
             }
        }
        
        if(handle && text) break;
    ...
    if (handle && text) {
        ...
        const created = !isNaN(t) ? Math.floor(t / 1000) : 0;
        seen.add(cid);
...
def _load_comments(page, max_scrolls, diag=None):
    """Muat komentar + BALASAN berjenjang di IG dengan kombinasi JS Scroll & Click."""
    if diag is None:
        diag = {}
        
    _dom = diag.setdefault("_dom_nodes", {})
    def _grab_dom():
        # Tangkap komentar root dari HTML awal agar tidak terlewat
        try:
    ...
    _sleep(4.0) # Tunggu popup postingan terbuka penuh
    _grab_dom()
    
    passes = max(1, int(max_scrolls or 1))
    for i in range(passes):
        print(f"      - Memuat komentar & balasan IG ({i+1}/{passes})...")
        
        # 1. Scroll panel komentar secara perlahan agar API terpicu
        _scroll_ig_comments(page)
        _sleep(1.5) 
        _grab_dom()
        
        # 2. Buka balasan berjenjang
        for _r in range(2):
            c = _click_ig_replies(page)
            diag["more_replies_clicked"] = diag.get("more_replies_clicked", 0) + c
            if not c:
                break
            _sleep(1.5)
            _grab_dom()
