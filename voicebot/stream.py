...
async def handle(websocket: WebSocket):
    ...
    # (cegah byte salam & jawaban saling menyisip bila greet() & processor()
    # kebetulan mengirim bersamaan).
                ...
                if data:
                    ...
                    for ev in ep.add(data, rms_min):
                        ...
                        if kind == "speech_start":
                            ...
                            elif not state["first_turn_done"] and state["bargein_on"]:
                                # #1: ucapan PERTAMA saat salam pembuka masih diputar --
                                # lindungi buffer agar tak terbuang; jawabannya menunggu
                                # salam selesai diputar (anti tumpang tindih #8).
                                state["capture"] = True
                            # saat talking (giliran berikutnya): interupsi ditangani kandidat/konfirmasi di bawah.
                        elif kind == "utterance":
                            # #5c: ucapan biasa ATAU ucapan penyela barge-in -> proses.
                            # #1: ucapan PERTAMA (saat salam pembuka diputar) juga
                            # diproses -- tidak dibuang. Jawabannya menunggu salam
                            # selesai diputar lebih dulu (anti tumpang tindih #8).
                            first_greet = (not state["first_turn_done"]
                                           and state["bargein_on"])
                            if not talking or state["capture"] or first_greet:
                                state["capture"] = False
                                state["first_turn_done"] = True
                                state["last_activity"] = now
                                state["idle_prompted"] = False
                                _log("utterance masuk ANTREAN proses (%d byte wav, antrean=%d, salam=%s)."
                                     % (len(ev[1]), queue.qsize() + 1, bool(first_greet and talking)))
    ...
    async def send_audio(b64, greeting_flag=False):
        ...
        async with audio_lock:
            ...
            # sebelum mengirim audio baru -> cegah tumpang tindih. Salam pembuka
            # (greeting_flag) & barge-in (interrupt) tak perlu menunggu. Ada timeout.
            if not greeting_flag:
                _wstart = time.time()
                while (not state["closed"] and not state["interrupt"]
                       and _is_talking()
