# -*- coding: utf-8 -*-
"""pipeline_routes.py — Pipeline analisis Dialogflow + Avaya (Step 1..16):
konteks request (Ctx/build_ctx), langkah step1..step11, integrasi avaya*,
dispatch aksi, unduhan, chat LLM, serta rute UI (/,/tools,/api/config,/api/chat).
Helper leaf (state/artefak, Google auth, HTTP, XLSX) dipindah ke pipeline_helpers.py.

Daftarkan dengan:  import pipeline_routes; pipeline_routes.register(app)
Helper di pipeline_helpers.py juga di-inject ke awe_routes.register() dari web_app.py.
"""
import os
import re
import io
...
#     pipeline_helpers.py. Perilaku tidak berubah. Diimpor ke namespace ini agar
#     pemanggilan `pipeline_routes.<helper>` dari modul lain (mis. web_app.py ->
#     awe_routes) tetap bekerja seperti sebelumnya. ---
...
def step10_build(cfg, ctx):
    # Sumber data: (1) unggahan analis kalau ada — Excel hasil Step 9 yang sudah
    # diedit manual di luar aplikasi; kalau tidak ada, (2) artefak Step 9 di server.
    up = ctx.file("xlsx_file")
    if up is not None:
        b = up[0]
        with open(os.path.join(run_dir(cfg, ctx.run), "step10_source.xlsx"), "wb") as f:
            f.write(b)
    else:
        state = load_state(cfg, ctx.run)
        s9 = state["steps"].get("9")
    ...
    if "Analisis MKTA" not in wb.sheetnames:
        raise Exception('Sheet "Analisis MKTA" tidak ada. Jalankan Step 9 dulu, atau unggah file yang memuat sheet "Analisis MKTA".')
    am = read_sheet(wb["Analisis MKTA"])
    H = am["headers"]
    ...
    summary = {"status": "Selesai", "baris_LM": len(lm_rows) - 1, "baris_Pembaruan": len(pem_rows) - 1,
               "sumber": ("Unggahan analis" if up is not None else "Artefak Step 9"),
