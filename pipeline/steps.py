# -*- coding: utf-8 -*-
"""pipeline_steps.py — Implementasi langkah pipeline (Step 1..16) + Avaya +
dispatch aksi. Dipisah dari pipeline_routes.py agar file rute tetap ringkas.
Perilaku fungsi TIDAK berubah — dipindah apa adanya (kecuali Step 10 yang kini
mendukung unggah Excel hasil Step 9 yang diedit manual). Helper leaf ada di
pipeline_helpers.py.
"""
import os
import re
...
def step10_build(cfg, ctx):
    # Sumber data: (1) unggahan analis jika ada — Excel hasil Step 9 yang sudah
    # diedit manual di luar aplikasi; jika tidak, (2) artefak Step 9 di server.
    up = ctx.file("xlsx_file")
    if up is not None:
        b = up[0]
