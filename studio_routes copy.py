...
def register(app, *, base_dir, render_page, llm_client, kctx, xlsx_mime):
    ...
    def _llm(output_type, doc_text, question, filename):
        gctx = _global_ctx(question or filename)
    ...
    async def api_studio_generate(request: Request):
        ...
        try:
            # ================= MULAI PERUBAHAN =================
            result = await run_in_threadpool(_generate, docid, output_type, question, request)
            
            # Potong output teks markdown sebelum dikirim ke UI browser
            max_preview = 1500
            if "markdown" in result and isinstance(result["markdown"], str):
                if len(result["markdown"]) > max_preview:
                    result["markdown"] = result["markdown"][:max_preview] + "\n\n... [TEKS PREVIEW DIPOTONG KARENA TERLALU PANJANG] ..."
